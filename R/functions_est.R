...
#' set.seed(123)
#' dataset <- lvhml_ex_data(J = 100, nset = "s", Kstar = 3)
#' attach(dataset)
#' out <- lvhml_est(Y, R, X, Kset = 1:10, par = FALSE, n.cores = 1, Asymp = FALSE, Silent = FALSE, Z = array(0, dim = c(0, 0, 0)))
#'
#' @export
lvhml_est <- function(Y, R, X , Kset = 1:10, par = FALSE, n.cores = 1, Asymp = FALSE, Silent = FALSE, Z = array(0, dim = c(0, 0, 0))) {
  ...
  for (i in seq_along(Kset)) {
    ...
    out <- lvhml_est_fit_K(Y, Ymatlog, R, XZ, X, XZmat, indices_R, lengths_R, Tp, K, cov_len, px, N, J, init_list$Tinit, init_list$Uinit, par, n.cores, Silent)
...
#'
#'@export
lvhml_norm = function(J,N,cov_len,px,tA,Theta,Gamma,tBeta,X){
