...
#nset: sets the size of N relative to J, "s": N = 5J, "L":N = 10J
#Kstar: True latent dimension
example_data.func <- function(J=100,nset="s", Kstar =3){
  if(nset =="s"){
    N = 5*J
  }
  if(nset =="L"){
...
genpar.func <- function(Kstar,N,J,Tp){
  ...
  Stars = norm.func(J, N, cov_len, px, tA, Theta, Gamma, tBeta,X)
  tAstar = Stars$tAstar
