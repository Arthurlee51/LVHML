...
#' @return A list structure that can be used as input for `lvhml_est` and true values of parameters for comparison.
...
#'@export
lvhml_ex_data <- function(J=100,nset="s", Kstar =3){
  if(nset =="s"){
    N = 5*J
  }
  if(nset =="L"){
...
genpar.func <- function(Kstar,N,J,Tp){
  ...
  Stars = lvhml_norm(J, N, cov_len, px, tA, Theta, Gamma, tBeta,X)
  tAstar = Stars$tAstar
